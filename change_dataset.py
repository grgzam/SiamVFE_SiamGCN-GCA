...
from utils import makedirs, files_exist, to_list, find_file, find_file_synthetic
...
class ChangeDataset(InMemoryDataset):
    def __init__(self, root, train=True, clearance = 3, ignore_labels=[], transform=None, pre_transform=None, pre_filter=None):        
        ...
        self.labels = osp.join(root, 'labeled_point_lists_train/2016-2020')
    ...
    def process_set(self, dataset):        
        csv_files = sorted(glob.glob(osp.join(self.labels, '*.csv')))
...
class ChangeDataset_synthetic(InMemoryDataset):
    def __init__(self, root, train=True, clearance = 3, ignore_labels=[], transform=None, pre_transform=None, pre_filter=None):
        self.root = root
        self.train = train
        self.clearance = clearance
        self.ignore_labels = ignore_labels
        self.data_time_a = osp.join(root, 'time_a')
        self.data_time_b = osp.join(root, 'time_b')
        self.labels = osp.join(root, 'labeled_point_lists_train_syn')
    ...
    def process_set(self, dataset):
        csv_files = sorted(glob.glob(osp.join(self.labels, '*.csv')))
        files_time_a = glob.glob(osp.join(self.data_time_a, '*.las'))
        files_time_b = glob.glob(osp.join(self.data_time_b, '*.las'))
        ...
        for file in csv_files:
            ...
            scene_num = int(re.search(r'\d+', file).group())

            f_time_a = find_file_synthetic(files_time_a, scene_num)
            f_time_b = find_file_synthetic(files_time_b, scene_num)
            ...
            points_time_a, h16 = load_las(f_time_a)
            points_time_b, h20 = load_las(f_time_b)
            ...
            for center, label in zip(centers, label_names):
                ...
                x1 = torch.tensor(extract_area(points_time_a, center[0:2], self.clearance), dtype=torch.float)
                data = Data(x=x1)
                data.x2 = torch.tensor(extract_area(points_time_b, center[0:2], self.clearance), dtype=torch.float)
                data.y = torch.tensor([label])
                data.scene_num = torch.tensor([int(scene_num)])
                data_list.append(data)
